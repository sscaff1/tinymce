...
describe('phantom.tinymce.themes.silver.window.WindowManagerTest', () => {
  ...
  const pAssertSinkStructure = async (asserter: (sink: SugarElement<HTMLElement>) => void) => {
    const sink = await UiFinder.pWaitFor('Looking for sink', SugarBody.body(), '.mce-silver-sink') as SugarElement<HTMLElement>;
    asserter(sink);
  };
  ...
  const pTest = async (conf: Dialog.DialogSpec<any>, asserter: (sink: SugarElement<HTMLElement>) => void, drag: boolean = false) => {
    ...
    await pAssertSinkStructure(asserter);
    await pTeardown();
  };
