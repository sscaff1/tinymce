...
describe('phantom.tinymce.themes.silver.sketchers.SilverMenubar Test', () => {
  ...
  const getSink = () => SelectorFind.descendant(SugarDocument.getDocument(), '.mce-silver-sink').getOrDie('Cannot find sink element');
  ...
  const assertActiveToggleItemHasOneCheckmark = (itemText: string) => {
    UiFinder.findIn(getSink(), '.tox-selected-menu [role=menuitemcheckbox]:contains("' + itemText + '")').getOrDie();
